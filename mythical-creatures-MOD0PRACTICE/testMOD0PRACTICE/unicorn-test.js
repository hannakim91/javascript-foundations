...
// new keyword with class invocation will create a class
  it('should have a name', function() {
    var unicorn = new Unicorn('Bree');
    assert.equal(unicorn.name, 'Bree');
  });
// assert - building block for any assertion - .equal(actual code, expected code) method is available on assertion
  it('should be able to have a different name', function() {
    var unicorn = new Unicorn('Pam');
    assert.equal(unicorn.name, 'Pam');
  });
// new unicorn instance can have a new name
...
// isWhite is a method we need to create
